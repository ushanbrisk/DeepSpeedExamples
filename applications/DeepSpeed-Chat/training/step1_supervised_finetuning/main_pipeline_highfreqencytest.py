...
from dschat.utils.data.distill_data_utils import create_dataset_lukedai_test_add_math

#copy from main_pipeline.py, only for high frequent test, changing different data, model for sft, distillation
def parse_args():
...
def main():
    ...
    # Prepare the data
    train_phase = 1
    train_dataset = create_dataset_lukedai_test_add_math(
    ...
    for step in range(
            args.num_train_epochs * num_update_steps_per_epoch - 1):  # -1 is importtant , abandon last residual to avoid error
        # for step in range(1):  # -1 is importtant , abandon last residual to avoid error
        torch.cuda.empty_cache()
        start1 = time.time()
        print_rank_0(
            f"step {step}, progress: {(step * 1.0) / (args.num_train_epochs * num_update_steps_per_epoch)}",
            args.global_rank)
