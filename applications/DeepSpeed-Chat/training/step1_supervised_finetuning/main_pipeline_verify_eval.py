...
def main():
    ...
    train_dataset_fake, eval_dataset_fake = create_prompt_dataset_0(
        args.is_eval,
        args.local_rank,
        args.data_path,
        args.data_split,
    ...
    #start of fake generator
    seed_fake = 1111
    generator_fake = torch.Generator()
    generator_fake.manual_seed(seed_fake)

    seed_fake_eval = 1111
    generator_fake_eval = torch.Generator()
    generator_fake_eval.manual_seed(seed_fake_eval)
    #end of fake generator
    ...
    #start of fake sampler
    train_sampler = RandomSampler(data_source=train_dataset, generator=generator_fake)
    eval_sampler = RandomSampler(data_source=train_dataset_fake, generator=generator_fake_eval)
    #end of fake sampler
    ...
    eval_dataloader = DataLoader(train_dataset_fake, **dataloader_params_fake)
    ...
    # start of eval dataloader
    eval_dataloader = iter(deepspeed.utils.RepeatingLoader(eval_dataloader))
    #end of eval dataloader
    ...
    for step in range(args.num_train_epochs * num_update_steps_per_epoch-1):  #-1 is importtant , abandon last residual to avoid error
        ...
        loss_eval = engine.eval_batch(data_iter=eval_dataloader)
